            ...
            world.getServer().runAsync(() -> {
                if (isVanillaDim(worldKey)) {
                    ...
                    if(ModConfigs.VAULT_COOP_ONLY.IS_COOP_ONLY){
                        ...
                        players.removeIf(seek -> (seek.getTeam()!=raiders)&&(seek!=player)&&seek.getServerWorld()!=player.getServerWorld()); // Remove if not a raider, the person activating it, and not in the same world.
